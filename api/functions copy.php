...
// $username = 'adronsof_admin';
// $password = 'adronsoft@2022';
// $dbname = 'adronsof_test_new';
$servername = 'localhost';
$username = 'root';
$password = '23988725';
$dbname = 'ttg-photostorage-new';

// define('APP_URL', 'http://localhost:8080');
define('APP_URL', 'http://adronsoft.in');
define('MEDIA_URL', APP_URL . '/');
...
function convertFilesUrl($filesData)
{
  foreach ($filesData as $key => $singleFile) {
    $filename = 'file' . $key + 1;
    $filesData[$key]->$filename = MEDIA_URL . $singleFile->$filename;
  }
  return $filesData;
}
function convertFilesUrl2($filesData)
{
  foreach ($filesData as $key => $singleFile) {
    $filesData[$key]['file'] = MEDIA_URL . $singleFile['file'];
  }
  return $filesData;
}
//-- 2. Delete user --//
...
function add_post($response)
{
  ...
  $sql1 = "INSERT INTO `ttg_post` (`userid`,`files`,`time`,`description`,`uid`,`crn`,`device`,`defect`,`device_type`) VALUES ('$userid','$files','$time_stamp','$description','$uid','$crn','$device','$defect','$device_type')";
  // print_r($sql1);
  // die();
  $result = $conn->query($sql1);
  // print_r('database result => ' . json_encode($result));
  // die();
  if ($result) {
    log_acivity('add', $uid, 'post', '');
    $allDefects = explode(',', $defect);
...
function pagination($totalitem, $pageitem, $pid)
{
  ...
  $pa .= '</select></form></div>';
  if (isset($_GET['s'])) {
    $pa .= "<script> document.getElementById('pgn" . $divid . "').scrollIntoView(); </script>";
  }
  $pa .= "<script> function pagi() { document.getElementById('mynop').submit(); }</script>";
...
function add_shipment($response)
{
  ...
  $sql1 = "INSERT INTO ttg_ship(userid,  crn,  files,  description,  time,  device,  ship_time,  logistic_company,  vahicle_type,  vahicle_container,
            vahicle_number,  box_condition,  supervisor_name,  supervisor_photo,  supervisor_sign,  note,  hash,  no_of_staff,  no_of_box,  no_of_pallets,
            no_of_devices,  no_of_vahicle,  supervisor_ph_no,  input_time,  is_reject,  declr_tick,  org_ship_time,  box_seal,  logistic_waybill)
            VALUES ('$userid',  '$crn',  '$files',  '$description',  '$time_stamp',  '$device',  '$ship_time',  '$logistic_company',  '$vahicle_type',  '$vahicle_container',
            '$vahicle_number',  '$box_condition',  '$supervisor_name',  '$supervisor_photo',  '$supervisor_sign',  '$note',  '$hash',  '$no_of_staff',  '$no_of_box',  '$no_of_pallets',
            '$no_of_devices',  '$no_of_vahicle',  '$supervisor_ph_no',  '$input_time',  '$is_reject',  '$declr_tick',  '$org_ship_time',  '$box_seal',  '$logistic_waybill')";
