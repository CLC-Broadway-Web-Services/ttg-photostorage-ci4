...
if(!isset($_GET['s']) OR !$_SESSION['mshipdata'])
{
$loadeddata=load_users($_POST['email'],'ship', $country);
        $_SESSION['mshipdata']= $loadeddata;	
}
else
{
	$loadeddata=$_SESSION['mshipdata'];
}
...
?>
<form id="deleteform" action="?p=mship" method="post" style="display:none">
    ...
    <div class="page_2box">
        <div>
        <h5 class="title_h5">Search Shipping Staff</h5><hr /><br />
        <p>Search shipping staff by Staff ID, Name, Email, Phone Number & Country</p>
        <form  method="POST" action="?p=mship" onsubmit="return checkdata();">
        ...
        <div>
        <h5 class="title_h5">Add Shipping Staff</h5><hr /><br />
        <p>Add new shipping staff here</p>
        
        <button  onclick="open_adduser('','ship')"  class="searchbtndate"><i class="fa fa-plus" aria-hidden="true"> Add</i></button>
        </div>
    </div>
    ...
    <div class="page_1box">
        <h5 class="title_h5">Shipping Staff List</h5><hr /><br />
        <?php echo $rtv ?>
    </div>
