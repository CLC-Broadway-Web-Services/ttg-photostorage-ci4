...
if(!isset($_GET['s']) OR !$_SESSION['madmindata'])
{
$loadeddata=load_users($_POST['email'],'admin', $country);
        $_SESSION['madmindata']= $loadeddata;	
}
else
{
	$loadeddata=$_SESSION['madmindata'];
}
...
?>
<form id="deleteform" action="?pq=madmin" method="post" style="display:none">
...
<div class="page_2box">
    <div>
    <h5 class="title_h5">Search Admin</h5><hr /><br />
    <p>Search admins by Admin ID, Name, Email, Phone Number & Country</p>
    <form  method="POST" action="?pq=madmin" onsubmit="return checkdata();">
    ...
    <div>
    <h5 class="title_h5">Add Admin</h5><hr /><br />
    <p>Add new admin here</p>
    
        <button onclick="open_adduser('','admin')" class="searchbtndate"><i class="fa fa-plus" aria-hidden="true"> Add</i></button>
    </div>
</div>

<div class="page_1box">
    <h5 class="title_h5">Admin List</h5><hr /><br />
    <?php echo $rtv ?>
</div>
